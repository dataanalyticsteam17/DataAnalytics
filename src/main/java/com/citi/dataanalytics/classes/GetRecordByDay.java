    ...
    public static ArrayList<Record> acessdatabyday(String start_date, String end_date,String symbol) throws IOException,java.text.ParseException {
        ...
        while (start.before(end) || start.equals(end)){
            ...
            for (int i=0;i<ls.size();i++){
               if( Float.parseFloat(ls.get(i).split(",")[3])>high) high =Float.parseFloat(ls.get(i).split(",")[3]);
               if (Float.parseFloat(ls.get(i).split(",")[4])<low) low =Float.parseFloat(ls.get(i).split(",")[4]);
            }
